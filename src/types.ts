...
// ============================================================================
// MULTI-AGENT MORNING BRIEFING (06:00 AM) & BETA TESTING READINESS TYPES
// ============================================================================

export type AgentId = 'sentinel-6' | 'lex-regulator' | 'aethelgard' | 'betaflight' | 'roundtable';
...
export interface MorningBriefingSchedule {
  enabled: boolean;
...
export interface MorningBriefingReport {
  ...
  preMarketRiskPosture: 'GREEN - NOMINAL' | 'YELLOW - ELEVATED' | 'ORANGE - HIGH VIGILANCE' | 'RED - DEFENSE HALT';
  executiveSummary: string;
  keyHighlights: string[];
  agentContributions: AgentContribution[];
  betaReleaseBlockers: BetaReleaseBlocker[];
  marketOpeningCountdown: string;
}

export interface BetaReadinessCheckItem {
  id: string;
  category: 'Forensic Engine' | 'Security & MFA' | 'Compliance & Legal' | 'Latency & FIX Protocol' | 'AI & Gemini SLA' | 'Data & Provenance';
  name: string;
  description: string;
  status: 'PASS' | 'WARN' | 'FAIL' | 'IDLE' | 'RUNNING';
  latencyMs?: number;
  details: string;
  statutoryRef?: string;
...
export interface BetaTesterInvite {
  ...
  organization: string;
  role: UserRole;
  status: 'INVITED' | 'ACTIVE' | 'FEEDBACK_SUBMITTED';
  accessKey: string;
  invitedDate: string;
